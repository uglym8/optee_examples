 ...
 * - configure the DES3 flavour from a command.
 ...
 */
struct des3_cipher {
	uint32_t algo;			/* DES3 flavour */
	uint32_t mode;			/* Encode or decode */
	uint32_t key_size;		/* DES3 key size in byte */
	TEE_OperationHandle op_handle;	/* DES3 ciphering operation */
...
{
	switch (param) {
	case TA_DES3_MODE_ENCODE:
		*mode = TEE_MODE_ENCRYPT;
		return TEE_SUCCESS;
	case TA_DES3_MODE_DECODE:
...
/*
 * Process command TA_DES3_CMD_PREPARE. API in des3_ta.h
 *
				  ...
				  TEE_Param params[4])
{
	...
	sess = (struct des3_cipher *)session;
	...
	DMSG("sess->algo = 0x%x", sess->algo);

	res = ta2tee_key_size(params[1].value.a, &sess->key_size);
	if (res != TEE_SUCCESS)
		return res;

	DMSG("sess->key_size = 0x%x", sess->key_size);

	res = ta2tee_mode_id(params[2].value.a, &sess->mode);
	if (res != TEE_SUCCESS)
		return res;

	DMSG("sess->mode = 0x%x", sess->mode);

	/*
	 * Ready to allocate the resources which are:
	 * - an operation handle, for an DES3 ciphering of given configuration
	...
	/* Allocate operation: DES3-CBC, mode and size from params */
	...
	if (res != TEE_SUCCESS) {
		EMSG("Failed to allocate operation. res = 0x%x", res);
	...
	res = TEE_AllocateTransientObject(TEE_TYPE_DES3,
					  sess->key_size * 8,
					  &sess->key_handle);
	if (res != TEE_SUCCESS) {
...
/*
 * Process command TA_DES3_CMD_SET_KEY. API in des3_ta.h
 */
static TEE_Result set_des3_key(void *session, uint32_t param_types,
				TEE_Param params[4])
{
	const uint32_t exp_param_types =
...
/*
 * Process command TA_DES3_CMD_SET_IV. API in des3_ta.h
 */
static TEE_Result reset_des3_iv(void *session, uint32_t param_types,
				TEE_Param params[4])
{
	const uint32_t exp_param_types =
		TEE_PARAM_TYPES(TEE_PARAM_TYPE_MEMREF_INPUT,
...
/*
 * Process command TA_DES3_CMD_CIPHER. API in des3_ta.h
 */
...
{
	switch (cmd) {
	case TA_DES3_CMD_PREPARE:
		return alloc_resources(session, param_types, params);
	case TA_DES3_CMD_SET_KEY:
		return set_des3_key(session, param_types, params);
	case TA_DES3_CMD_SET_IV:
		return reset_des3_iv(session, param_types, params);
	case TA_DES3_CMD_CIPHER:
